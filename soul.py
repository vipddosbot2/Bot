...
TOKEN = '8370800620:AAFKbfPiTyhFUSr0cQx3D88qjbcev07oLRM'
FORWARD_CHANNEL_ID = -1003476877991
...
def create_inline_keyboard():
    ...
    button3 = types.InlineKeyboardButton(
        text="♻️ 𝗝𝗢𝗜𝗡 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ♻️", url="https://t.me/+84dzjkgSdKtkYmE1")
    button1 = types.InlineKeyboardButton(text="☣️ 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 ☣️",
        url="https://t.me/VIPXOWNER9")
    markup.add(button3)
    markup.add(button1)
    return markup
...
def extend_and_clean_expired_users():
    ...
    for user_id, user_data in users_data.items():
        ...
        time_approved_str = user_data.get("time_approved", "")
        username = user_data.get("username", "Unknown User")
        days = user_data.get("days", 0)
        approving_admin_id = user_data.get("approved_by")
        ...
        if valid_until_str:
            try:
                ...
                if now > valid_until_datetime:
                    ...
                    logging.info(f"User {user_id} marked for removal. 🗑️")
    ...
    for user_id in expired_users:
        ...
        logging.info(f"User {user_id} has been removed from memory. 🗑️")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Please contact the owner for assistance: @VIPXOWNER9.*", 
                                       reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
    ...
    try:
        ...
        if not user_data:
            ...
            button2 = types.InlineKeyboardButton(
                text="💸 𝗣𝗿𝗶𝗰𝗲 𝗟𝗶𝘀𝘁 💸", url="https://t.me/+84dzjkgSdKtkYmE1")
            markup.add(button1)
            markup.add(button2)
        else:
